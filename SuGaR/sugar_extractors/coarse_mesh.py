...
def extract_mesh_from_coarse_sugar(args):
    ...
    if use_vanilla_3dgs:
        ...
        with torch.no_grad():
            ...
            scales = sugar._scales 
            scales_r = scales.norm(dim=-1).cpu().numpy()
            print("σ 范围: min", scales_r.min(), "max", scales_r.max(), "mean", scales_r.mean())
            # sugar._scales[:] *= 0.2
    ...
    if not use_marching_cubes:
        ...
        with torch.no_grad():
            ...
            # density_th = densities.mean().item() * 0.95
            ...
            dens_np = densities.cpu().numpy().ravel()
            print("min, max, mean:", dens_np.min(), dens_np.max(), dens_np.mean())
